...
export function Table({
  ...
  const tableValue = useMemo(() => ({ bleed, dense, grid, striped }), [bleed, dense, grid, striped]);

  return (
    <TableContext.Provider value={tableValue as React.ContextType<typeof TableContext>}>
...
export function TableRow({
  ...
  const rowValue = useMemo(() => ({ href, target, title }), [href, target, title]);

  return (
    <TableRowContext.Provider value={rowValue as React.ContextType<typeof TableRowContext>}>
      <tr
        {...props}
        className={clsx(
          className,
