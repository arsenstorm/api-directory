...
export async function POST(req: NextRequest) {
	...
	await returnIsEnabled("ageandgender");

	let { estimated, actual = null } = await getEstimatedCost("ageandgender");
	...
	try {
		...
		if (!apiResponse.ok) {
			throw new Error("Failed to get response from ageandgender.");
		}
		...
		if (data?.image) {
			...
			const imageName = `${requestId}-ageandgender.png`;
			...
			if (error) {
				return NextResponse.json(
					{
						message: "Failed to save ageandgender image to storage.",
					},
					{
						status: 400,
					},
